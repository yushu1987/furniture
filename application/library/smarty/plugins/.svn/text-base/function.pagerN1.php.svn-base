...
function smarty_function_pagerN1($arrParams, &$smarty)
{
    ...
    if ($arrParams['page'] > 1)
    {
        if(isset($arrParams['nosprintf']) && $arrParams['nosprintf'])
            ...
            if(($arrParams['page']-1)==1){
                $strPager.= '<a class="next-page" style="margin-right:0" href="' . sprintf($arrParams['pagelink'], $arrParams['page']-1) . '">上一页</a>';
            }else{
                $strPager.= '<a class="next-page" style="margin-right:0" href="' . sprintf($arrParams['pagelink'].$arrParams['suffix'], $arrParams['page']-1) . '">上一页</a>';
    ...
    for ($i = $intPageStart; $i <= $intPageEnd; $i++)
    {
        ...
        else
        {
            if( isset($arrParams['nosprintf']) && $arrParams['nosprintf']){
                ...
                if($i==1){
                    $strPager .= '<a href="' . $arrParams['pagelink'] . '">' . $i . '</a>';
                }else{
                    $strPager.= '<a href="' . sprintf($arrParams['pagelink'].$arrParams['suffix'], $i) . '"> '. $i . '</a>';
                }
    ...
    if ($arrParams['page'] < $intPages)
    {
        if( isset( $arrParams['nosprintf']) && $arrParams['nosprintf'])
            ...
            $strPager.= '<a class="next-page" href="' . sprintf($arrParams['pagelink'].$arrParams['suffix'], $arrParams['page']+1) . '">下一页</a>';
    }
